...
export default function App() {
  ...
  const parseHtmlToDocFormat = (htmlString, sourceUrl) => {
    ...
    const mainContent = htmlDoc.querySelector('main, article, #mw-content-text, .content, #content') || htmlDoc.body;
    if (!mainContent) throw new Error("No readable content found on this site.");

    const elements = mainContent.querySelectorAll('p, h1, h2, h3, li');
    const maxElements = 150; // Truncate extremely long articles
    let count = 0;

    elements.forEach((el, idx) => {
      if (count >= maxElements) return;
      // Skip structural noise
      if (el.closest('nav, header, footer, script, style, table, .infobox, .reflist, .reference, .navbox')) return;

      const blockId = `block_${idx}`;
      const runs = [];

      const traverse = (node, activeUrl) => {
        if (node.nodeType === Node.TEXT_NODE) {
          const txt = node.textContent;
          // Avoid pushing totally empty whitespace blocks, but preserve internal spacing
          if (txt.trim()) {
            runs.push({ id: randomId('text'), text: txt, url: activeUrl });
          }
        } else if (node.nodeType === Node.ELEMENT_NODE) {
          let nextUrl = activeUrl;
          if (node.tagName === 'A' && node.getAttribute('href')) {
            const href = node.getAttribute('href');
            // Ignore internal hash jumps and JS
            if (!href.startsWith('#') && !href.startsWith('javascript:')) {
              try {
                nextUrl = new URL(href, sourceUrl).href;
                links.push({
                  id: randomId('link'),
                  url: nextUrl,
                  title: node.textContent.trim() || nextUrl,
                  docId: webDocId,
                });
              } catch (e) {
                // Invalid URL, ignore
          ...
          node.childNodes.forEach((child) => traverse(child, nextUrl));
        }
      };
      ...
      if (runs.length > 0) {
        blocks.push({ id: blockId, type: 'paragraph', runs });
        count++;
      }
    });
