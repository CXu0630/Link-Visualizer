...
const processInputString = (inputStr) => {
  return inputStr
    .split(/[\s,]+/)
    .filter(Boolean)
    .map((val) => {
      const docMatch = val.match(GOOGLE_DOC_REGEX);
      if (docMatch) return { type: 'gdoc', id: docMatch[1], original: val };
      if (URL_REGEX.test(val)) return { type: 'website', url: val, original: val };
      if (val.length > 10 && !val.includes(' ')) return { type: 'gdoc', id: val, original: val };
      return null;
    })
    .filter(Boolean);
};
...
export default function App() {
  ...
  const handleSignOut = () => {
    if (user) window.google.accounts.oauth2.revokeToken(user.access_token);
    setUser(null);
    setAppState('unauthorized');
    setSourceDocs([]);
    setTargetDocs([]);
  ...
  // ---------------- HTML PARSER FOR GENERIC WEBSITES ----------------
  const parseHtmlToDocFormat = (htmlString, sourceUrl) => {
    const parser = new DOMParser();
    const htmlDoc = parser.parseFromString(htmlString, 'text/html');
    const title = htmlDoc.title || sourceUrl;
    const webDocId = randomId('webdoc');
    const links = [];

    // Target major content blocks to avoid massive navbars / footers
    let mainContent = htmlDoc.querySelector('main, article, #mw-content-text, .content, #content');
    
    // Fallback if no specific semantic wrapper is found
    if (!mainContent) {
      mainContent = htmlDoc.body;
    }

    // Clone content so we can mutate it freely
    const contentClone = mainContent.cloneNode(true);

    // Filter out common layout noise, menus, headers, and reference lists
    const noiseSelectors = [
      'script', 'style', 'nav', 'footer', 'header', 'aside', 
      '.infobox', '.reflist', '.reference', '.navbox', 
      'iframe', 'form', '.mw-editsection', '.toc', '.noprint'
    ];
    ...
    contentClone.querySelectorAll('img').forEach(img => {
      ...
      if (src) {
        try {
          img.src = new URL(src, sourceUrl).href;
        } catch(e) {}
      }
    });

    // Process Links, map them for the Graph Engine, and inject tracking IDs
    const anchors = Array.from(contentClone.querySelectorAll('a'));
    const MAX_LINKS = 150; // Truncate highly linked documents to keep graph sane
    let linkCount = 0;

    anchors.forEach(a => {
      const href = a.getAttribute('href');
      
      // Ignore anchors that are just javascript triggers or page jumps
      if (href && !href.startsWith('#') && !href.startsWith('javascript:')) {
        if (linkCount >= MAX_LINKS) {
           // Replace excess links with plain text
        ...
        try {
          const nextUrl = new URL(href, sourceUrl).href;
          const linkId = randomId('link');
          
          links.push({
            id: linkId,
            url: nextUrl,
            title: a.textContent.trim() || nextUrl,
            docId: webDocId,
          });
          
          // Inject exact attributes needed by GraphLayer tracker
          a.setAttribute('data-track-id', linkId);
          a.setAttribute('target', '_blank');
          a.setAttribute('rel', 'noopener noreferrer');
          a.className = 'text-emerald-600 bg-emerald-50/60 px-1 rounded hover:bg-emerald-100 transition-colors inline-block relative font-medium cursor-pointer';
          
          linkCount++;
        } catch (e) {
          // Invalid URL, replace with plain text
          const span = document.createElement('span');
          span.textContent = a.textContent;
          a.replaceWith(span);
        }
      } else {
        // Replace anchor with plain text
        const span = document.createElement('span');
        span.textContent = a.textContent;
        a.replaceWith(span);
      }
    ...
    return {
      ...
      blocks: [], // Empty blocks to ensure we don't break Google Doc format iterators
      links,
    };
  };

  // ---------------- GOOGLE DOC API PARSER ----------------
  ...
  const fetchResource = async (input) => {
    if (input.type === 'gdoc') {
      if (!user) throw new Error(`You must sign in to Google to read Docs (${input.id}).`);
      const response = await window.gapi.client.docs.documents.get({ documentId: input.id });
      return parseDocumentContent(response.result);
    } 
    
    if (input.type === 'website') {
      const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(input.url)}`;
      const response = await fetch(proxyUrl);
      if (!response.ok) throw new Error(`Failed to fetch website (${response.status})`);
      const data = await response.json();
      if (!data.contents) throw new Error("No readable content received. The site might be blocking proxies.");
      return parseHtmlToDocFormat(data.contents, input.url);
    }
  ...
  const fetchAndProcessDocs = async () => {
    const inputs = processInputString(docIdsInput);

    if (inputs.length === 0) {
      setError('Please enter valid Google Doc IDs or Website URLs.');
      return;
    }
    ...
    try {
      ...
      for (const input of inputs) {
        try {
          fetchedDocs.push(await fetchResource(input));
        } catch (e) {
          console.error(`Failed to fetch ${input.original}`, e);
          throw new Error(`Failed to parse ${input.original}: ${e.message}`);
        }
      }

      if (fetchedDocs.length === 0) {
        throw new Error('Could not find any of the provided documents or websites.');
      }
  ...
  const handleAddSingleDoc = async () => {
    const inputs = processInputString(newDocInput);
    if (inputs.length === 0) return;
    
    const input = inputs[0]; // Just take first if multiple pasted
    
    // Check dupe
    if (sourceDocs.some(d => d.id === input.id || d.url === input.url)) {
      alert("This document/website is already in the canvas.");
      return;
    }

    setIsAddingLoading(true);
    try {
      const fetchedDoc = await fetchResource(input);
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans flex flex-col overflow-hidden">
      {/* Dynamic Styling injected for natively rendering Web HTML */}
      <style dangerouslySetInnerHTML={{__html: `
        .raw-html-content h1, .raw-html-content h2, .raw-html-content h3 { font-weight: 700; color: #1e293b; margin-top: 1.5em; margin-bottom: 0.5em; }
        .raw-html-content h1 { font-size: 1.5em; }
        .raw-html-content h2 { font-size: 1.25em; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.25em; }
        .raw-html-content h3 { font-size: 1.1em; }
        .raw-html-content p { margin-bottom: 1em; line-height: 1.6; }
        .raw-html-content ul { list-style-type: disc; padding-left: 1.5em; margin-bottom: 1em; }
        .raw-html-content ol { list-style-type: decimal; padding-left: 1.5em; margin-bottom: 1em; }
        .raw-html-content blockquote { border-left: 4px solid #cbd5e1; padding-left: 1em; margin-left: 0; color: #475569; font-style: italic; }
        .raw-html-content img { max-width: 100%; height: auto; border-radius: 0.25rem; margin: 1em 0; }
        .raw-html-content table { border-collapse: collapse; width: 100%; margin-bottom: 1em; }
        .raw-html-content th, .raw-html-content td { border: 1px solid #cbd5e1; padding: 0.5em; }
        .raw-html-content pre { background: #f1f5f9; padding: 1em; border-radius: 0.25rem; overflow-x: auto; margin-bottom: 1em;}
        .raw-html-content code { font-family: monospace; background: #f1f5f9; padding: 0.1em 0.3em; border-radius: 0.2em; font-size: 0.9em; }
      `}} />
          ...
          {user ? (
            ...
            >
             ...
             appState === 'visualizing' && (
               <button
                onClick={handleAuth}
                className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-medium shadow-lg transition-all"
               >
                 <LogIn className="w-4 h-4" />
                 <span>Sign in for Docs</span>
               </button>
             )
          )}
        </div>
      ...
      <main className="flex-1 relative flex items-center justify-center overflow-hidden">
        {appState === 'unauthorized' && (
          <div className="max-w-md w-full p-8 bg-[#1e293b] rounded-2xl border border-slate-700 shadow-2xl text-center">
            ...
            <p className="text-slate-400 mb-8 text-sm leading-relaxed">
              To visualize links between your private Google Docs, we need read-only access. You can also skip this if you only want to parse public websites.
            </p>
            <div className="flex flex-col gap-3">
              ...
              <button
                onClick={() => setAppState('ready')}
                className="w-full flex items-center justify-center gap-3 py-3 px-6 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-300 rounded-xl font-bold transition-all"
              >
                Skip Auth (Websites Only) <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        )}
        ...
        {appState === 'ready' && (
          <div className="max-w-xl w-full p-10 bg-[#1e293b] rounded-2xl border border-slate-700 shadow-2xl relative overflow-hidden">
            ...
            <p className="text-slate-400 mb-8 text-sm">Paste Document IDs or Website URLs to parse their textual content and connect them.</p>

            <div className="mb-6">
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                Google Doc IDs or Website URLs (e.g., Wikipedia)
              </label>
              <textarea
                value={docIdsInput}
                ...
                placeholder="https://docs.google.com/document/d/..., https://en.wikipedia.org/wiki/Cybernetics"
              />
            </div>
        ...
        {appState === 'loading' && (
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-lg font-medium text-slate-300">Reconstructing content & connections...</p>
          </div>
        )}
              ...
              <div className="p-5 flex-1 overflow-y-auto flex flex-col gap-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Documents / Sites</span>
                    <button 
                  ...
                  {isAddingDoc && (
                    <div className="mb-4 flex flex-col gap-2 p-3 bg-slate-800/80 rounded-lg border border-slate-600 shadow-inner">
                      <input
                        ...
                        placeholder="Doc ID or Website URL..."
                  ...
                  <div className="flex flex-col gap-3">
                    {sourceDocs.map(doc => {
                      ...
                      const isWeb = doc.sourceType === 'website';
                      return (
                        <div key={doc.id} className={`flex items-center justify-between pl-3 border-l-2 ${isWeb ? 'border-emerald-500/50' : 'border-blue-500/50'}`}>
                          <div className="flex items-center gap-2 overflow-hidden pr-2">
                            {isWeb ? <Globe className="w-3.5 h-3.5 text-emerald-500 shrink-0" /> : <FileText className="w-3.5 h-3.5 text-blue-500 shrink-0" />}
    ...
    <div ref={wrapperRef} className="relative p-20 min-w-[2000px] min-h-[2000px]">
      ...
      <div className="absolute top-0 left-0">
        {visibleSourceDocs.map((doc, idx) => {
          const isWeb = doc.sourceType === 'website';
          const isSelected = selectedNodes.has(doc.id);
          const borderClass = isSelected 
             ? (isWeb ? 'border-emerald-500 ring-4 ring-emerald-500/40 z-30' : 'border-blue-500 ring-4 ring-blue-500/40 z-30')
             : 'border-slate-300 z-20';
          ...
          return (
            <div
              ...
              className={`absolute w-[850px] flex flex-col bg-slate-50 border rounded-xl shadow-2xl overflow-hidden node-element cursor-move transition-[box-shadow,border-color] duration-150 ${borderClass}`}
              style={{ 
            ...
            >
              ...
              <div className="px-5 py-4 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3 min-w-0">
                  {isWeb ? <Globe className="w-5 h-5 text-emerald-500 shrink-0" /> : <FileText className="w-5 h-5 text-blue-600 shrink-0" />}
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-colors shrink-0 ${isWeb ? 'text-slate-400 hover:text-emerald-600' : 'text-slate-400 hover:text-blue-600'}`}
                >
              ...
              <div 
                className="bg-white p-8 text-sm text-slate-800 leading-relaxed cursor-text overflow-y-auto"
                onMouseDown={(e) => e.stopPropagation()} 
                onScroll={calculatePositions}
              ...
              >
                {isWeb ? (
                  <div 
                    ...
                    onMouseEnter={calculatePositions}
                  />
                ) : (
